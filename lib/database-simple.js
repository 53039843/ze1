...
// In-memory storage for logs (non-persistent for serverless environment)
let inMemoryLogs = [];
// For demonstration purposes, users and orders will still use /tmp for now
const fs = require("fs");
...
const logOps = {
  ...
  add: (logData) => {
    ...
    if (inMemoryLogs.length > 1000) {
      inMemoryLogs.splice(1000);
    }
  },

  // 获取日志列表（分页）
  getList: (account, page = 1, pageSize = 20) => {
    const filtered = inMemoryLogs.filter(log => log.account === account);
  ...
  // 获取总数
  getCount: (account) => {
    return inMemoryLogs.filter(log => log.account === account).length;
  },
