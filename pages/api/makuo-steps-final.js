// 最终优化版本：使用api.3x.ink API的小米运动步数更新接口，支持自动回退到ZeppLife
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (makuoResult.shouldNotFallback) {
      ...
      return res.status(400).json(makuoResult);
    }

    // 第三步：回退到ZeppLife API
    console.log(`[${requestId}] 开始回退到ZeppLife API...`);
    const zeppResult = await callZeppLifeAPI(requestId, account, password, targetSteps);
    
    const duration = Date.now() - startTime;
    console.log(`[${requestId}] 请求处理完成，总耗时: ${duration}ms`);
 ...
 */
async function callZeppLifeAPI(requestId, account, password, targetSteps) {
  try {
    console.log(`[${requestId}] 正在调用ZeppLife API...`);

    // 登录获取token
    const { loginToken, userId } = await zeppLifeSteps.login(account, password);
    console.log(`[${requestId}] ZeppLife登录成功`);

    // 获取app token
    const appToken = await zeppLifeSteps.getAppToken(loginToken);
    console.log(`[${requestId}] ZeppLife获取appToken成功`);

    // 修改步数
    const result = await zeppLifeSteps.updateSteps(loginToken, appToken, targetSteps);
    console.log(`[${requestId}] ZeppLife步数更新成功`);
    ...
    console.error(`[${requestId}] ZeppLife API调用失败:`, error.message);
