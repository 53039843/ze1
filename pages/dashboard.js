...
const Dashboard = () => {
  ...
  const [accountModalVisible, setAccountModalVisible] = useState(true);
  ...
  const loadAllData = async (acc) => {
    ...
    try {
      ...
      const [statsRes, dailyRes, logsRes] = await Promise.all([
        axios.get(`/api/stats/user?account=${acc}`),
        axios.get(`/api/stats/daily?account=${acc}&days=7`),
        axios.get(`/api/stats/logs?account=${acc}&page=${currentPage}&pageSize=${pageSize}`)
      ]);
  ...
  useEffect(() => {
    if (account) {
      axios.get(`/api/stats/logs?account=${account}&page=${currentPage}&pageSize=${pageSize}`)
        .then(res => {
          if (res.data.success) {
  ...
  const columns = [
    ...
    },
    {
      ...
      key: 'balance_after',
      width: 100,
      render: (balance) => balance !== null ? `¥${balance.toFixed(4)}` : '-',
    },
  ];
    ...
    <>
      ...
      <div style={{ minHeight: '100vh', position: 'relative', zIndex: 1 }}>
        ...
        <div style={{ 
          ...
          >
            <Input
              placeholder="请输入您的Zepp Life账号"
              value={accountInput}
              onChange={(e) => setAccountInput(e.target.value)}
              onPressEnter={handleAccountConfirm}
              size="large"
            />
          </Modal>
            ...
            <>
              ...
              <div style={{ 
                ...
                <div>
                  ...
                  <p style={{ margin: '4px 0 0 0', color: '#666' }}>账号: {account}</p>
                </div>
                <Button 
                  icon={<ReloadOutlined />} 
